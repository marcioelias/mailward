...
const setActive = (row: AliasDomainRow): void => {
    ...
    toggle.post(`/alias-domains/${row.alias_domain}/active`, { preserveScroll: true })
}
...
<template>
    ...
    <AppLayout
        title="Alias domains"
        subtitle="A domain whose mail is delivered to the accounts of another."
    >
        <div class="me-stack">
            ...
            <div class="me-row me-row--between">
                <div class="me-row">
                    <MeInput v-model="search" type="search" placeholder="Search alias domains…" />
                ...
                <Link v-if="can.create" href="/alias-domains/create" class="me-btn me-btn--primary">
                    Add alias domain
                </Link>
            </div>

            <div v-if="!aliasDomains.data.length" class="me-empty">
                <p v-if="filters.search || filters.target_domain">Nothing matches that filter.</p>
            ...
            <table v-else class="me-table">
                ...
                <tbody>
                    <tr v-for="row in aliasDomains.data" :key="row.alias_domain">
                        <td><strong>{{ row.alias_domain }}</strong></td>
        ...
        >
            <p>
                Mail addressed to <strong>{{ pendingDeletion?.alias_domain }}</strong> will stop
                being delivered to <strong>{{ pendingDeletion?.target_domain }}</strong>.
            </p>
            <p>No account is deleted — this removes the routing, not the mailboxes.</p>
        </MeModal>
    </AppLayout>
</template>
